...
INLINE void omptarget_nvptx_TaskDescr::InitLevelZeroTaskDescr()
{
  ...
  */
  data.items.flags = 0; 
  data.items.nthreads = GetNumberOfProcsInTeam();; // threads: whatever was alloc by kernel
...
INLINE void omptarget_nvptx_TaskDescr::CopyData(omptarget_nvptx_TaskDescr *sourceTaskDescr)
{
  data.vect[0] = sourceTaskDescr->data.vect[0]; 
  data.vect[1] = sourceTaskDescr->data.vect[1]; 
} 

INLINE void omptarget_nvptx_TaskDescr::Copy(omptarget_nvptx_TaskDescr *sourceTaskDescr)
{
  CopyData(sourceTaskDescr);
  prev = sourceTaskDescr->prev;
} 

INLINE void omptarget_nvptx_TaskDescr::CopyParent(omptarget_nvptx_TaskDescr *parentTaskDescr)
{
  CopyData(parentTaskDescr);
  prev = parentTaskDescr;
}

INLINE void omptarget_nvptx_TaskDescr::CopyForExplicitTask(omptarget_nvptx_TaskDescr *parentTaskDescr)
{
  ...
  uint16_t tnum)
{
  ...
  data.items.flags |= TaskDescr_InPar | TaskDescr_IsParConstr; // set flag to parallel
  data.items.threadsInTeam = tnum; // set number of threads
}

INLINE void omptarget_nvptx_TaskDescr::CopyFromWorkDescr(omptarget_nvptx_TaskDescr *workTaskDescr)
{
  Copy(workTaskDescr);
  // overrwrite specific items;
...
INLINE omptarget_nvptx_TaskDescr *omptarget_nvptx_ThreadPrivateContext::GetTopLevelTaskDescr(int gtid)
{
...
INLINE void omptarget_nvptx_ThreadPrivateContext::InitThreadPrivateContext(int gtid)
{
...
{
  return omptarget_nvptx_teamContexts[GetOmpTeamId()];
}
...
{
  return omptarget_nvptx_threadPrivateContext.GetTopLevelTaskDescr(globalThreadId);
}
