...
//#define OMPTARGET_NVPTX_DEBUG (LD_SET_LOOP) // limit to loop printfs to save on cuda buffer
...
#define LT_FUSSY      (LT_SET_FUSSY)
  ...
  #define ASSERT0(_flag, _cond, _str)           { if (TON(_flag)) {assert(_cond);}}
  #define  ASSERT(_flag, _cond, _str, _args...) { if (TON(_flag)) {assert(_cond);}}
  ...
  #define ASSERT0(_flag, _cond, _str) { if (TON(_flag) && !(_cond)) { \
    printf("<b %3d, t %4d, w %2d, l %2d> ASSERT: " _str "\n", blockIdx.x, threadIdx.x, \
      threadIdx.x / warpSize, threadIdx.x & 0x1F); assert(_cond);}}
  #define ASSERT(_flag, _cond, _str, _args...) { if (TON(_flag) && !(_cond)) { \
    printf("<b %3d, t %4d, w %2d, l %d2> ASSERT: " _str "\n", blockIdx.x, threadIdx.x, \
      threadIdx.x / warpSize, threadIdx.x & 0x1F, _args); assert(_cond);}}
...
#define LW_FUSSY      (LW_SET_FUSSY)
  ...
  #define WARNING0(_flag, _str) { if (WON(_flag)) { \
    printf("<b %2d, t %4d, w %2d, l %2d> WARNING: " _str, blockIdx.x, threadIdx.x, \
      threadIdx.x / warpSize, threadIdx.x & 0x1F); }}
